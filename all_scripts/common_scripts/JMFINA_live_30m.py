...
INTERVAL = "30minute"
SYMBOL = "JMFINA"  # Stock symbol

# STATE
position = None
units = 0
position_file = f"{SYMBOL}.json"

logging.debug("Loading historical data from CSV...")
# Load historical data with indicators pre-calculated
historical_df = pd.read_csv(f"{SYMBOL}.csv", parse_dates=["datetime"])
...
def save_position():
    """Save current position state to JSON file named by symbol."""
    global position, units
    data = {"position": position, "units": units}
    with open(position_file, "w") as f:
        json.dump(data, f)
    logging.info(f"Saved state: {position}, Units: {units}")

def sync_broker_position():
    """Sync broker position for the symbol and update state."""
    global position, units
    try:
        ...
        if resp.get("Success") is None:
            ...
            save_position()
            return
        pos = next((p for p in resp["Success"] if p["stock_code"] == SYMBOL), None)
...
def check_strategy(row):
    ...
    if (row["RSI_D"] > 58 and
        ...
        (pd.isna(row.get("EMA_200_M")) or row["open"] > row.get("EMA_200_M"))):
...
# Tick handler for 30-min candles
def on_ticks(tick):
    ...
    try:
        ...
        logging.debug(f"Received 30-min tick for time: {ts}, price: {price}")
    except Exception as e:
        logging.warning(f"Invalid tick data: {tick} -> {e}")
        return

    # Use last row from historical_df for indicators; update price only
...
breeze.subscribe_feeds(
    exchange_code="NSE", stock_code=SYMBOL, expiry_date="", strike_price="",
    right="", product_type="cash", get_market_depth=False, get_exchange_quotes=True,
    interval=INTERVAL
)

logging.info(f"🚀 Live trading active using {INTERVAL} candles (paper mode: {'OFF' if IS_LIVE else 'ON'})")
