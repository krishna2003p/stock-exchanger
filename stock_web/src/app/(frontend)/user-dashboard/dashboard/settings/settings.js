           ...
           <aside className="col-span-12 md:col-span-3">
            <div className="bg-white border rounded-xl p-3">
              ...
              <SidebarGroup
                title="Profile"
                items={[
                  { icon: <UserIcon />, label: "Profile", active: true, desc: "Manage your personal information and profile settings" },
                ]}
              />
              <SidebarGroup
                title="Security"
                items={[
                  { icon: <LockIcon />, label: "Security", desc: "Password, 2FA, API keys, and security settings" },
                ]}
              />
              <SidebarGroup
                title="Notifications"
                items={[
                  { icon: <BellIcon />, label: "Notifications", desc: "Configure email, push, and SMS notifications" },
                ]}
              />
              <SidebarGroup
                title="Appearance"
                items={[
                  { icon: <AppearanceIcon />, label: "Appearance", desc: "Theme, layout, language, and accessibility settings" },
                ]}
              />
              <SidebarGroup
                title="Trading"
                items={[
                  { icon: <TradingIcon />, label: "Trading", desc: "Default trading settings and risk management" },
                ]}
              />
              <SidebarGroup
                title="Bots"
                items={[
                  { icon: <BotsIcon />, label: "Bots", desc: "Bot parameters, behavior, and monitoring settings" },
                ]}
              />
              <SidebarGroup
                title="Privacy"
                items={[
                  { icon: <PrivacyIcon />, label: "Privacy", desc: "Data sharing, profile privacy, and tracking settings" },
                ]}
              />
              <SidebarGroup
                title="Data"
                items={[
                  { icon: <DataIcon />, label: "Data", desc: "Data export, retention, and account management" },
                ]}
              />
              <SidebarGroup
                title="Connections"
                items={[
                  { icon: <ConnectionsIcon />, label: "Connections", desc: "Connected exchanges, wallets, and third-party services" },
                ]}
              />
            </div>
          </aside>
    ...
    <div className="mb-2">
      ...
      <div className="flex flex-col">
        {items.map((it) => (
          <button
            key={it.label}
            className={`text-left rounded-lg px-3 py-2 mb-1 border hover:bg-gray-50 ${
              it.active ? "bg-gray-100 border-gray-200" : "border-transparent"
            }`}
          >
